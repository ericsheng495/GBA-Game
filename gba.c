...
void drawFullScreenDMA(const u16 *image) {
  // TODO: IMPLEMENT
  for (int r = 0; r < SCREEN_HEIGHT; r++) {
    DMA[3].src = &image[OFFSET(r, 0, SCREEN_WIDTH)];
    DMA[3].dst = &videoBuffer[OFFSET(r, 0, SCREEN_WIDTH)];
    DMA[3].cnt = SCREEN_WIDTH | DMA_ON | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT;
  }
}
